...
public class Piece {
    ...
    public List<Point> getAvailableCaptures() {
        List<Point> availableCaptures = new ArrayList<>();
        for (int targetCol = 0; targetCol < 8; targetCol++) {
            for (int targetRow = 0; targetRow < 8; targetRow++) {
                if (canMove(targetCol, targetRow) && checkValidSquare(targetCol, targetRow) == 2) {
                    availableCaptures.add(new Point(targetCol, targetRow));
                }
            }
        }
    ...
    public List<Point> getAvailableMoves() {
        ...
        // Kiểm tra tất cả các ô trong phạm vi di chuyển của quân cờ
        for (int targetCol = 0; targetCol < 8; targetCol++) {
            for (int targetRow = 0; targetRow < 8; targetRow++) {
                if (canMove(targetCol, targetRow) && checkValidSquare(targetCol, targetRow) == 1) {
                    availableMoves.add(new Point(targetCol, targetRow));
                }
            }
        }
